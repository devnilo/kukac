...
async function getCashAmount() {
    ...
    const purchase = document.querySelector('#inputPurchase').value || 0
    if (purchase.includes(','))
        purchase = purchase.split(',')[0]
    if (purchase.includes('.'))
        purchase = purchase.split('.')[0]

    const payment = document.querySelector('#inputPayment').value || 0
    if (payment.includes(','))
        payment = payment.split(',')[0]
    if (payment.includes('.'))
        payment = payment.split('.')[0]

    const asyncGetCashAmount = () => {
        try {
...
async function showCashResults() {
    try {
